...
def add_date_time(entries):
   ...
   command = "ffmpeg -i \"{input}\" -vf \"drawtext=timecode='{tc}':r={fps}:x=(w-tw)/2:y=h-(lh):fontsize=30: text={take_date}: box=1:boxcolor=black@0.8:fontcolor=white\" -c:a copy \"{output}\"".format(input=video_in, output=video_out, tc=timecode, fps=framerate, take_date=video_date + '-')
